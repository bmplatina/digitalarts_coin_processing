...
void draw() {
    ...
    textWithStroke(checkIncreasing(0) + Integer.toString(coinPriceChanges[0]), 7, 192.5, height/2+150, 36, #FFFFFF, #FF0000);
    textWithStroke("디아챗", 7, 494.5, 547.0, 36, #000000, #FFFFFF);
    textWithStroke(checkIncreasing(1) + Integer.toString(coinPriceChanges[1]), 7, 494.5, height/2+150, 36, #FFFFFF, #FF0000);
    textWithStroke("디아테크", 7, width/2, 547.0, 36, #000000, #FFFFFF);
    textWithStroke(checkIncreasing(2) + Integer.toString(coinPriceChanges[2]), 7, width/2, height/2+150, 36, #FFFFFF, #FF0000);
    textWithStroke("디아엔터", 7, 1098.5, 547.0, 36, #000000, #FFFFFF);
    textWithStroke(checkIncreasing(3) + Integer.toString(coinPriceChanges[3]), 7, 1098.5, height/2+150, 36, #FFFFFF, #FF0000);
    textWithStroke("디아건축", 7, 1398.5, 547.0, 36, #000000, #FFFFFF);
    textWithStroke(checkIncreasing(4) + Integer.toString(coinPriceChanges[4]), 7, 1398.5, height/2+150, 36, #FFFFFF, #FF0000);
